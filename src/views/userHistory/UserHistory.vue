...
export default {
  components: {
                HistoryTop: () => import('./Top'), 
                HistoryList: () => import('./List'),
  ...
  methods: {
    ...
    getList() {
      this.$http.get(API.HISTORY_LIST,{ 
        ...
        if (res.data.success == true){
          ...
          this.totalCnt = String(res.data.topCount.totalCnt)
          this.searchCnt = String(res.data.topCount.searchCnt)
          this.sessionCnt = 'GPU : ' + res.data.topCount.sessionCnt + ' volta'
          this.appCnt = 'CPU : ' + res.data.topCount.appCpu + ' 개<br />MEM : ' + res.data.topCount.appMem + ' Mb<br />GPU : ' + res.data.topCount.appGpu + ' Gb'
          this.apiCnt = String(res.data.topCount.apiCnt)
          this.usingUserCnt = String(res.data.topCount.usingUserCnt)
        }
      }, err => {
          console.log(err);
      })
    ...
    search (radio, modelType, user) {
      this.modelType = String(modelType)
      this.filter = radio
